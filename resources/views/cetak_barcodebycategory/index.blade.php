        ...
        <div class="col-lg-3 md-3 mb-4">
            <div class="card bg-dark text-white text-xs shadow">
                <div class="card-body">
                    ...
                    <form action="{{ route('cetak_barcode.store') }}" method="POST" class="form-prevent" id="formID">
                        @csrf
                        @method('POST')
                        <input type="hidden" name="material_id" value="{{ $material->id }}">
                        <input type="hidden" name="user_id" value="{{ Auth()->user()->id }}">
                        <button type="submit" class="btn btn-warning btn-sm text-dark" id="submitID{{ $material->id }}" onclick="this.form.submit(); this.disabled=true;">
                            Cetak
