...
<div class="container-fluid">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active" aria-current="page">Tambah {{ ucfirst("analisa Posbrix") }} {{ $posbrix->name }}</li>
        </ol>
    </nav>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h5 class="m-0 font-weight-bold text-primary">Tambah {{ ucfirst("analisa Posbrix") }} {{ $posbrix->name }}</h5>
        </div>
        <div class="card-body">
            <form action="{{ route("analisa_posbrix.store") }}" method="post">
                @csrf
                <input type="hidden" name="user_id" value="{{ Auth()->user()->id }}">
                <input type="hidden" name="posbrix_id" value="{{ $posbrix->id }}">
                <p>
